...
export const initialPublicFilters: PublicFilters = {
  topic: 'all',
  format: 'all',
  ruleSet: 'all',
  sessionRange: 'all'
...
export type PublicRecentRow = {
  topic: string
  score: number
  latestSession: string
}

export type PublicTrendRow = {
  session: string
  topic: string
  count: number
}
...
export type PublicBlockRow = {
  ...
  answerRange: string
  confidence: PublicBlockHit['confidence']
  formats: string
  matchedKeywords: string
}

export type PublicFormatRow = {
  format: string
  count: number
  rate: number
}

export type PublicAggregateSummary = {
  totalCount: number
  totalBlocks: number
  hasLowConfidence: boolean
  topicRanking: PublicRankingRow[]
  recentRanking: PublicRecentRow[]
  trendRows: PublicTrendRow[]
  blockRows: PublicBlockRow[]
  formatRows: PublicFormatRow[]
...
function sortResults(results: PublicAnalysisResult[]): PublicAnalysisResult[] {
  return [...results].sort(
    (a, b) =>
...
export function aggregatePublicResults(
  ...
  const trendRows: PublicTrendRow[] = []
  const blockRows: PublicBlockRow[] = []
  ...
  sorted.forEach((result, resultIndex) => {
    ...
    for (const hit of result.blockHits) {
      ...
      blockRows.push({
        session: result.examSession,
        subjectName: result.ruleSet.subjectName,
        block: hit.block,
        heading: hit.heading,
        topic: hit.topic_l1,
  ...
  const availableRuleSets = Array.from(new Map(
    sorted.map((result) => [result.ruleSet.code, {
      code: result.ruleSet.code,
