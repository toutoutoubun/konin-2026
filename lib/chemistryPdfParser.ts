...
import {
  aggregateChemistryFormatCounts,
  analyzeChemistryBlocks,
  chemistryRuleSet,
  detectChemistryBlocks,
  detectChemistryExamSession,
  detectChemistryExamYear,
  estimateChemistryQuestionCount,
  type ChemistryAnalysisResult
} from './chemistryTagMapper'

export type ChemistryParseProgress = {
  fileName: string
...
export async function parseChemistryPdf(
  file: File,
  onProgress?: (progress: ChemistryParseProgress) => void
  ...
  const examYear = detectChemistryExamYear(fullText, file.name)
  const examSession = detectChemistryExamSession(fullText, file.name)

  onProgress?.({ fileName: file.name, status: 'tagging', message: '大問構成、頻出分野、出題形式を解析しています。' })

  const blockHits = analyzeChemistryBlocks(rawText)
  const detectedBlocks = detectChemistryBlocks(rawText)
  const questionCount = estimateChemistryQuestionCount(fullText)
  const formatCounts = aggregateChemistryFormatCounts(blockHits)
...
export async function parseChemistryPdfs(
  files: File[],
  onProgress?: (progress: ChemistryParseProgress) => void
): Promise<ChemistryAnalysisResult[]> {
  const results: ChemistryAnalysisResult[] = []
  for (const file of files) {
    const result = await parseChemistryPdf(file, onProgress)
    results.push(result)
  }
  return results
}
