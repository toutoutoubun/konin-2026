...
import {
  aggregatePhysicsFormatCounts,
  analyzePhysicsBlocks,
  detectPhysicsBlocks,
  detectPhysicsExamSession,
  detectPhysicsExamYear,
  estimatePhysicsQuestionCount,
  physicsRuleSet,
  type PhysicsAnalysisResult
} from './physicsTagMapper'

export type PhysicsParseProgress = {
  fileName: string
...
export async function parsePhysicsPdf(
  file: File,
  onProgress?: (progress: PhysicsParseProgress) => void
  ...
  const examYear = detectPhysicsExamYear(fullText, file.name)
  const examSession = detectPhysicsExamSession(fullText, file.name)

  onProgress?.({ fileName: file.name, status: 'tagging', message: '大問構成、頻出分野、出題形式を解析しています。' })

  const blockHits = analyzePhysicsBlocks(rawText)
  const detectedBlocks = detectPhysicsBlocks(rawText)
  const questionCount = estimatePhysicsQuestionCount(fullText)
  const formatCounts = aggregatePhysicsFormatCounts(blockHits)
...
export async function parsePhysicsPdfs(
  files: File[],
  onProgress?: (progress: PhysicsParseProgress) => void
): Promise<PhysicsAnalysisResult[]> {
  const results: PhysicsAnalysisResult[] = []
  for (const file of files) {
    const result = await parsePhysicsPdf(file, onProgress)
    results.push(result)
  }
  return results
}
