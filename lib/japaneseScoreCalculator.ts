...
export type JapaneseFilters = {
  topic: string
  format: string
  sessionRange: string
}

export const initialJapaneseFilters: JapaneseFilters = {
  topic: 'all',
  format: 'all',
  sessionRange: 'all'
}

export type JapaneseRankingRow = {
  topic: string
  count: number
  rate: number
}

export type JapaneseRecentRow = {
  topic: string
  score: number
  latestSession: string
}

export type JapaneseTrendRow = {
  session: string
  topic: string
  count: number
}

export type JapaneseBlockRow = {
  session: string
  block: string
  heading: string
  topic: string
  ...
  answerRange: string
  confidence: JapaneseBlockHit['confidence']
  formats: string
  matchedKeywords: string
}

export type JapaneseFormatRow = {
  format: string
  count: number
  rate: number
}

export type JapaneseAggregateSummary = {
  totalCount: number
  totalBlocks: number
  hasLowConfidence: boolean
  topicRanking: JapaneseRankingRow[]
  recentRanking: JapaneseRecentRow[]
  trendRows: JapaneseTrendRow[]
  blockRows: JapaneseBlockRow[]
  formatRows: JapaneseFormatRow[]
...
function sortResults(results: JapaneseAnalysisResult[]): JapaneseAnalysisResult[] {
  return [...results].sort(
    (a, b) =>
...
export function filterJapaneseResults(
  results: JapaneseAnalysisResult[],
  filters: JapaneseFilters
): JapaneseAnalysisResult[] {
...
export function aggregateJapaneseResults(
  results: JapaneseAnalysisResult[]
): JapaneseAggregateSummary {
  const sorted = sortResults(results)
  const topicCounts = new Map<string, number>()
  const recentScores = new Map<string, { score: number; latestSession: string }>()
  const formatCounts = new Map<string, number>()
  const trendRows: JapaneseTrendRow[] = []
  const blockRows: JapaneseBlockRow[] = []
