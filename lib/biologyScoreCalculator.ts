...
export const initialBiologyFilters: BiologyFilters = {
  topic: 'all',
  format: 'all',
  sessionRange: 'all'
}

export type BiologyRankingRow = {
  topic: string
  count: number
  rate: number
}

export type BiologyRecentRow = {
  topic: string
  score: number
  latestSession: string
}

export type BiologyTrendRow = {
  session: string
  topic: string
  count: number
}

export type BiologyBlockRow = {
  session: string
  block: string
  heading: string
  topic: string
  ...
  answerRange: string
  confidence: BiologyBlockHit['confidence']
  formats: string
  matchedKeywords: string
}

export type BiologyFormatRow = {
  format: string
  count: number
  rate: number
}

export type BiologyAggregateSummary = {
  totalCount: number
  totalBlocks: number
  hasLowConfidence: boolean
  topicRanking: BiologyRankingRow[]
  recentRanking: BiologyRecentRow[]
  trendRows: BiologyTrendRow[]
  blockRows: BiologyBlockRow[]
  formatRows: BiologyFormatRow[]
...
function sortResults(results: BiologyAnalysisResult[]): BiologyAnalysisResult[] {
  return [...results].sort(
    (a, b) =>
...
export function filterBiologyResults(
  results: BiologyAnalysisResult[],
  filters: BiologyFilters
): BiologyAnalysisResult[] {
...
export function aggregateBiologyResults(
  results: BiologyAnalysisResult[]
): BiologyAggregateSummary {
  const sorted = sortResults(results)
  const topicCounts = new Map<string, number>()
  const recentScores = new Map<string, { score: number; latestSession: string }>()
  const formatCounts = new Map<string, number>()
  const trendRows: BiologyTrendRow[] = []
  const blockRows: BiologyBlockRow[] = []
