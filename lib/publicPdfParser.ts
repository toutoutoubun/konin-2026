...
import {
  aggregatePublicFormatCounts,
  analyzePublicBlocks,
  detectPublicBlocks,
  detectPublicExamSession,
  detectPublicExamYear,
  detectPublicRuleSet,
  estimatePublicQuestionCount,
  type PublicAnalysisResult
} from './publicTagMapper'

export type PublicParseProgress = {
  fileName: string
...
export async function parsePublicPdf(
  file: File,
  onProgress?: (progress: PublicParseProgress) => void
  ...
  const examYear = detectPublicExamYear(fullText, file.name)
  const examSession = detectPublicExamSession(fullText, file.name)
  const ruleSet = detectPublicRuleSet(fullText, file.name)

  onProgress?.({ fileName: file.name, status: 'tagging', message: '大問構成、頻出分野、出題形式を解析しています。' })

  const blockHits = analyzePublicBlocks(rawText, ruleSet)
  const detectedBlocks = detectPublicBlocks(rawText)
  const questionCount = estimatePublicQuestionCount(fullText)
  const formatCounts = aggregatePublicFormatCounts(blockHits)
...
export async function parsePublicPdfs(
  files: File[],
  onProgress?: (progress: PublicParseProgress) => void
): Promise<PublicAnalysisResult[]> {
  const results: PublicAnalysisResult[] = []
  for (const file of files) {
    const result = await parsePublicPdf(file, onProgress)
    results.push(result)
  }
  return results
}
