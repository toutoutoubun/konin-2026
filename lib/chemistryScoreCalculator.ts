...
export type ChemistryFilters = {
  topic: string
  format: string
  sessionRange: string
}

export const initialChemistryFilters: ChemistryFilters = {
  topic: 'all',
  format: 'all',
  sessionRange: 'all'
}

export type ChemistryRankingRow = {
  topic: string
  count: number
  rate: number
}

export type ChemistryRecentRow = {
  topic: string
  score: number
  latestSession: string
}

export type ChemistryTrendRow = {
  session: string
  topic: string
  count: number
}

export type ChemistryBlockRow = {
  session: string
  block: string
  heading: string
  topic: string
  ...
  answerRange: string
  confidence: ChemistryBlockHit['confidence']
  formats: string
  matchedKeywords: string
}

export type ChemistryFormatRow = {
  format: string
  count: number
  rate: number
}

export type ChemistryAggregateSummary = {
  totalCount: number
  totalBlocks: number
  hasLowConfidence: boolean
  topicRanking: ChemistryRankingRow[]
  recentRanking: ChemistryRecentRow[]
  trendRows: ChemistryTrendRow[]
  blockRows: ChemistryBlockRow[]
  formatRows: ChemistryFormatRow[]
...
function sortResults(results: ChemistryAnalysisResult[]): ChemistryAnalysisResult[] {
  return [...results].sort(
    (a, b) =>
...
export function filterChemistryResults(
  results: ChemistryAnalysisResult[],
...
export function aggregateChemistryResults(
  results: ChemistryAnalysisResult[]
): ChemistryAggregateSummary {
  const sorted = sortResults(results)
  const topicCounts = new Map<string, number>()
  const recentScores = new Map<string, { score: number; latestSession: string }>()
  const formatCounts = new Map<string, number>()
  const trendRows: ChemistryTrendRow[] = []
  const blockRows: ChemistryBlockRow[] = []
