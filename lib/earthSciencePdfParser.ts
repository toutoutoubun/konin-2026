...
import {
  aggregateEarthScienceFormatCounts,
  analyzeEarthScienceBlocks,
  detectEarthScienceBlocks,
  detectEarthScienceExamSession,
  detectEarthScienceExamYear,
  earthScienceRuleSet,
  estimateEarthScienceQuestionCount,
  type EarthScienceAnalysisResult
} from './earthScienceTagMapper'

export type EarthScienceParseProgress = {
  fileName: string
...
export async function parseEarthSciencePdf(
  file: File,
  onProgress?: (progress: EarthScienceParseProgress) => void
): Promise<EarthScienceAnalysisResult> {
  ...
  const examYear = detectEarthScienceExamYear(fullText, file.name)
  const examSession = detectEarthScienceExamSession(fullText, file.name)

  onProgress?.({ fileName: file.name, status: 'tagging', message: '大問構成、頻出分野、出題形式を解析しています。' })

  const blockHits = analyzeEarthScienceBlocks(rawText)
  const detectedBlocks = detectEarthScienceBlocks(rawText)
  const questionCount = estimateEarthScienceQuestionCount(fullText)
  const formatCounts = aggregateEarthScienceFormatCounts(blockHits)
...
export async function parseEarthSciencePdfs(
  files: File[],
  onProgress?: (progress: EarthScienceParseProgress) => void
): Promise<EarthScienceAnalysisResult[]> {
  const results: EarthScienceAnalysisResult[] = []
  for (const file of files) {
    const result = await parseEarthSciencePdf(file, onProgress)
    results.push(result)
  }
  return results
}
