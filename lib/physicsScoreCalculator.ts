...
export type PhysicsFilters = {
  topic: string
  format: string
  sessionRange: string
}

export const initialPhysicsFilters: PhysicsFilters = {
  topic: 'all',
  format: 'all',
  sessionRange: 'all'
}

export type PhysicsRankingRow = {
  topic: string
  count: number
  rate: number
}

export type PhysicsRecentRow = {
  topic: string
  score: number
  latestSession: string
}

export type PhysicsTrendRow = {
  session: string
  topic: string
  count: number
}

export type PhysicsBlockRow = {
  session: string
  block: string
  heading: string
  topic: string
  ...
  answerRange: string
  confidence: PhysicsBlockHit['confidence']
  formats: string
  matchedKeywords: string
}

export type PhysicsFormatRow = {
  format: string
  count: number
  rate: number
}

export type PhysicsAggregateSummary = {
  totalCount: number
  totalBlocks: number
  hasLowConfidence: boolean
  topicRanking: PhysicsRankingRow[]
  recentRanking: PhysicsRecentRow[]
  trendRows: PhysicsTrendRow[]
  blockRows: PhysicsBlockRow[]
  formatRows: PhysicsFormatRow[]
...
function sortResults(results: PhysicsAnalysisResult[]): PhysicsAnalysisResult[] {
  return [...results].sort(
    (a, b) =>
...
export function filterPhysicsResults(
  results: PhysicsAnalysisResult[],
  filters: PhysicsFilters
): PhysicsAnalysisResult[] {
...
export function aggregatePhysicsResults(
  results: PhysicsAnalysisResult[]
): PhysicsAggregateSummary {
  const sorted = sortResults(results)
  const topicCounts = new Map<string, number>()
  const recentScores = new Map<string, { score: number; latestSession: string }>()
  const formatCounts = new Map<string, number>()
  const trendRows: PhysicsTrendRow[] = []
  const blockRows: PhysicsBlockRow[] = []
