...
import ChemistryPDFUploader from '@/components/ChemistryPDFUploader'
import ChemistryFilterPanel from '@/components/ChemistryFilterPanel'
import {
  aggregateChemistryResults,
  filterChemistryResults,
  initialChemistryFilters,
  type ChemistryFilters
} from '@/lib/chemistryScoreCalculator'
import type { ChemistryAnalysisResult } from '@/lib/chemistryTagMapper'
import {
  Bar,
  BarChart,
  CartesianGrid,
...
export default function ChemistryPage() {
  const [results, setResults] = useState<ChemistryAnalysisResult[]>([])
  const [filters, setFilters] = useState<ChemistryFilters>(initialChemistryFilters)
  const [error, setError] = useState('')

  const filteredResults = useMemo(
    () => filterChemistryResults(results, filters),
    [results, filters]
  )
  const summary = useMemo(
    () => aggregateChemistryResults(filteredResults),
    [filteredResults]
  )
  const allSummary = useMemo(
    () => aggregateChemistryResults(results),
    [results]
  )
  ...
  return (
    <>
      ...
      <Header navItems={[
        ...
        { label: 'PDF選択', href: '#chemistry-upload-title' },
        { label: '集計', href: '#chemistry-ranking-title' },
        { label: 'フィルタ', href: '#chemistry-filter-title' },
      ...
      <nav className="mx-auto mt-4 flex max-w-7xl gap-2 px-4 text-sm text-ink/70 md:px-10" aria-label="パンくずリスト">
        ...
        <span aria-hidden="true">/</span>
        <span aria-current="page">化学基礎頻出分析</span>
      </nav>

      <main id="main-content" className="mx-auto max-w-7xl px-4 pb-20 md:px-10" tabIndex={-1}>
        <section className="py-12 md:py-20" aria-labelledby="chemistry-hero-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.22em]">CHEMISTRY PAST EXAM ANALYZER</p>
          <h1 id="chemistry-hero-title" className="mt-4 max-w-5xl font-mincho text-[48px] font-bold leading-none tracking-normal">
            <ruby>化学<rt>かがく</rt></ruby><ruby>基礎<rt>きそ</rt></ruby><ruby>頻出<rt>ひんしゅつ</rt></ruby><ruby>分析<rt>ぶんせき</rt></ruby>
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-relaxed sm:mt-7 sm:text-xl">
            ユーザーが文科省公式ページから取得した化学基礎PDFを端末内で解析し、頻出分野・年度別推移・大問構成を可視化します。PDFはサーバーへ送信せず、問題文や設問文は再掲載しません。
          </p>
          <p className="mt-3 max-w-3xl">
            化学基礎は20の解答番号を5大問程度に分けて出題されます。「問1〜問4に答えよ。」などの大問見出しと、本文内の化学用語を組み合わせて分野と出題形式を判定します。
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:gap-4" aria-label="主要操作">
            <a className="hard-button button-like bg-blue px-5 py-3 text-center text-white no-underline" href="#chemistry-upload-title">PDFを分析する</a>
        ...
        {error && (
          <section className="panel mt-8 border-orange bg-paper p-6" aria-live="polite" aria-labelledby="chemistry-error-title">
            <h2 id="chemistry-error-title" className="font-mincho text-3xl font-bold">解析できませんでした</h2>
            <p className="mt-2">{error}</p>
            <p className="mt-2">文科省公式ページから取得した化学基礎PDFを選び直してください。</p>
          </section>
        )}

        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-status-title" aria-live="polite">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">ANALYSIS STATUS</p>
          <h2 id="chemistry-status-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">解析結果</h2>
          {hasResults ? (
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-ranking-title">
          ...
          <h2 id="chemistry-ranking-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            <ruby>頻出<rt>ひんしゅつ</rt></ruby><ruby>分野<rt>ぶんや</rt></ruby>ランキング
          </h2>
          <p className="mt-3 max-w-3xl">
            大問ごとに最も強く出た分野を集計します。酸・塩基と酸化還元のように同じ「化学反応」見出しになりやすい大問は、本文キーワードで切り分けます。
          </p>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-recent-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION B</p>
          <h2 id="chemistry-recent-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
          ...
          <div className="mt-5 overflow-x-auto" role="region" aria-label="化学基礎の近年頻出ランキング表" tabIndex={0}>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-block-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION C</p>
          <h2 id="chemistry-block-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            <ruby>大問<rt>だいもん</rt></ruby><ruby>構成<rt>こうせい</rt></ruby>
          </h2>
          <p className="mt-3 max-w-3xl">
            各PDFの大問ごとに、小問数、解答番号範囲、判定分野、出題形式を整理します。
          </p>
          <div className="mt-5 overflow-x-auto" role="region" aria-label="化学基礎の大問構成表" tabIndex={0}>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-trend-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION D</p>
          <h2 id="chemistry-trend-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-format-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">FORMAT DISTRIBUTION</p>
          <h2 id="chemistry-format-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="chemistry-meta-title">
          ...
          <h2 id="chemistry-meta-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            注記
          </h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <div className="border-2 border-ink bg-cream p-4">
              <h3 className="text-xl font-bold">化学基礎の解析方法</h3>
              <p className="mt-3">
                公式PDFの見出しで大問を分割し、各大問の解答番号範囲と小問数を推定します。
              </p>
              <p className="mt-2 text-sm">
                分野は、化学と人間生活・物質の構成粒子、物質と化学結合、物質量と化学反応式、化学反応（酸・塩基）、化学反応（酸化還元）のキーワードスコアで分類します。
              </p>
            </div>
          </div>
          <p className="mt-5">
...
function RankingTable({ rows }: { rows: Array<{ topic: string; count: number; rate: number }> }) {
  return (
    <div className="overflow-x-auto" role="region" aria-label="化学基礎の頻出分野ランキング表" tabIndex={0}>
...
function FormatTable({ rows }: { rows: Array<{ format: string; count: number; rate: number }> }) {
  return (
    <div className="mt-5 overflow-x-auto" role="region" aria-label="化学基礎の出題形式分布表" tabIndex={0}>
      <table className="w-full min-w-[420px] border-collapse bg-paper" role="table">
