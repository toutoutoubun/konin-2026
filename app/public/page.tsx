...
import PublicPDFUploader from '@/components/PublicPDFUploader'
...
import type { PublicAnalysisResult } from '@/lib/publicTagMapper'
import {
  Bar,
  BarChart,
  CartesianGrid,
...
export default function PublicPage() {
  const [results, setResults] = useState<PublicAnalysisResult[]>([])
  const [filters, setFilters] = useState<PublicFilters>(initialPublicFilters)
  const [error, setError] = useState('')

  const filteredResults = useMemo(
    () => filterPublicResults(results, filters),
    [results, filters]
  )
  const summary = useMemo(
    () => aggregatePublicResults(filteredResults),
    [filteredResults]
  )
  const allSummary = useMemo(
    () => aggregatePublicResults(results),
    [results]
  )
  ...
  return (
    <>
      ...
      <Header navItems={[
        ...
        { label: 'PDF選択', href: '#public-upload-title' },
        { label: '集計', href: '#public-ranking-title' },
        { label: 'フィルタ', href: '#public-filter-title' },
      ...
      <main id="main-content" className="mx-auto max-w-7xl px-4 pb-20 md:px-10" tabIndex={-1}>
        <section className="py-12 md:py-20" aria-labelledby="public-hero-title">
          ...
          <h1 id="public-hero-title" className="mt-4 max-w-5xl font-mincho text-[48px] font-bold leading-none tracking-normal">
            <ruby>公共<rt>こうきょう</rt></ruby><ruby>頻出<rt>ひんしゅつ</rt></ruby><ruby>分析<rt>ぶんせき</rt></ruby>
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-relaxed sm:mt-7 sm:text-xl">
            ユーザーが文科省公式ページから取得した公共PDFを端末内で解析し、頻出分野・年度別推移・大問構成を可視化します。PDFはサーバーへ送信せず、問題文や設問文は再掲載しません。
          </p>
          <p className="mt-3 max-w-3xl">
            令和6年度以降の公共を主対象にします。現代社会・倫理・政治・経済は旧課程の参考データとして読み込めますが、ランキングを見るときは制度区分フィルタで公共だけに絞れます。
          </p>
        ...
        {error && (
          <section className="panel mt-8 border-orange bg-paper p-6" aria-live="polite" aria-labelledby="public-error-title">
            <h2 id="public-error-title" className="font-mincho text-3xl font-bold">解析できませんでした</h2>
            <p className="mt-2">{error}</p>
            <p className="mt-2">文科省公式ページから取得した公共PDFを選び直してください。</p>
          </section>
        )}

        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-status-title" aria-live="polite">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">ANALYSIS STATUS</p>
          <h2 id="public-status-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">解析結果</h2>
          {hasResults ? (
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-ranking-title">
          ...
          <h2 id="public-ranking-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            <ruby>頻出<rt>ひんしゅつ</rt></ruby><ruby>分野<rt>ぶんや</rt></ruby>ランキング
          </h2>
          <p className="mt-3 max-w-3xl">
            大問ごとに最も強く出た分野を集計します。公共PDFだけを見る場合は、下のフィルタで「新課程：公共」を選びます。
          </p>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-recent-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION B</p>
          <h2 id="public-recent-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
          ...
          <div className="mt-5 overflow-x-auto" role="region" aria-label="公共の近年頻出ランキング表" tabIndex={0}>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-block-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION C</p>
          <h2 id="public-block-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            <ruby>大問<rt>だいもん</rt></ruby><ruby>構成<rt>こうせい</rt></ruby>
          </h2>
          <p className="mt-3 max-w-3xl">
            各PDFの大問ごとに、制度区分、小問数、解答番号範囲、判定分野、出題形式を整理します。
          </p>
          <div className="mt-5 overflow-x-auto" role="region" aria-label="公共の大問構成表" tabIndex={0}>
            <table className="w-full min-w-[1080px] border-collapse bg-paper" role="table">
              <caption className="py-3 text-left font-bold">試験回別の大問構成。</caption>
              <thead className="bg-ink text-cream">
                <tr>
                  <th scope="col" className="p-3 text-left">試験回</th>
                  <th scope="col" className="p-3 text-left">区分</th>
              ...
              <tbody>
                {summary.blockRows.length > 0 ? (
                  ...
                  <tr><td colSpan={9} className="p-3">該当データはない：PDF解析後にデータを表示します。</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-trend-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">SECTION D</p>
          <h2 id="public-trend-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-format-title">
          <p className="font-serifDisplay text-sm uppercase tracking-[.18em]">FORMAT DISTRIBUTION</p>
          <h2 id="public-format-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
        ...
        <section className="panel mt-8 p-6 md:p-8" aria-labelledby="public-meta-title">
          ...
          <h2 id="public-meta-title" className="mt-2 font-mincho text-2xl font-bold sm:text-4xl md:text-6xl">
            注記
          </h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
...
function RankingTable({ rows }: { rows: Array<{ topic: string; count: number; rate: number }> }) {
  return (
    <div className="overflow-x-auto" role="region" aria-label="公共の頻出分野ランキング表" tabIndex={0}>
...
function FormatTable({ rows }: { rows: Array<{ format: string; count: number; rate: number }> }) {
  return (
    <div className="mt-5 overflow-x-auto" role="region" aria-label="公共の出題形式分布表" tabIndex={0}>
      <table className="w-full min-w-[420px] border-collapse bg-paper" role="table">
