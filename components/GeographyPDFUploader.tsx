...
import { parseGeographyPdfs, type GeoParseProgress } from '@/lib/geographyPdfParser'
import type { GeoAnalysisResult } from '@/lib/geographyTagMapper'
import SkeletonLoader from './SkeletonLoader'

type Props = {
  onComplete: (results: GeoAnalysisResult[]) => void
  onError: (message: string) => void
}

export default function GeographyPDFUploader({ onComplete, onError }: Props) {
  const inputRef = useRef<HTMLInputElement | null>(null)
  const [dragging, setDragging] = useState(false)
  const [loading, setLoading] = useState(false)
  const [progress, setProgress] = useState<GeoParseProgress | null>(null)
  ...
  const handleFiles = async (fileList: FileList | File[]) => {
    ...
    onError('')
    try {
      const results = await parseGeographyPdfs(files, setProgress)
      onComplete(results)
    } catch (error) {
      const message =
  ...
  return (
    <section className="panel p-6 md:p-8" aria-labelledby="geo-upload-title">
      <div className="mb-6">
        ...
        <p id="geo-upload-help" className="mt-3 max-w-3xl">
          文科省公開の地理・地理A・地理B過去問PDFをアップロードすると分析します。複数ファイルを同時に追加できます。PDFはサーバーへ送信せず、ブラウザ上で処理します。
        </p>
      </div>
      ...
      <div id="geo-upload-status" className="mt-5" aria-live="polite">
        {loading ? (
          <SkeletonLoader label={progress?.message ?? 'PDFアップロード中'} />
        ) : (
          <p className="border-2 border-ink bg-cream p-4">
            未解析。PDFを選択すると、解析したファイル名と検出した試験回・科目名・制度区分を表示します。
          </p>
        )}
      </div>
    </section>
