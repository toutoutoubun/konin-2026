...
import { parseEnglishPdfs, type ParseProgress } from '@/lib/pdfParser'
...
export default function PDFUploader({ onComplete, onError }: Props) {
  ...
  const handleFiles = async (fileList: FileList | File[]) => {
    ...
    onError('')
    try {
      const results = await parseEnglishPdfs(files, setProgress)
      onComplete(results)
    } catch (error) {
  ...
  return (
    <section className="panel p-6 md:p-8" aria-labelledby="upload-title">
      ...
      >
        ...
        />
        <div>
          <span className="mx-auto mb-4 grid h-20 w-20 place-items-center rounded-full border-2 border-ink bg-orange font-serifDisplay text-xl">PDF</span>
